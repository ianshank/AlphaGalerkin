...
- **Re-measured the AC7 adequacy-gate risk an independent adversarial review of the implementation plan surfaced**: the task-zero spike's headline rate-separation numbers (`-1.256`/`-0.710`) came from the spike's own stronger, non-production ZZ estimator, not the weaker production one this slice actually extracts. Measured through the real production primitives on the real L-shaped Poisson benchmark: adaptive **-1.2515**, uniform **-0.6710** — both comfortably inside the planned Slice D thresholds. No recalibration needed. **Corrected in Slice D**: this line originally read "adaptive -1.322, uniform -0.671", which mixed two different fitting windows (the adaptive figure was fitted over `(200, 2600)`, the uniform over `(200, 4000)`, because the narrower window cannot hold three uniform points at all — see Slice D). Both figures above are over the single corrected `RATE_FIT_DOF_RANGE = (200, 4000)`, and both are now **bounded by a committed test** (`tests/research/test_amr_arena_interpretability.py` asserts adaptive ≤ `-1.10` and uniform ∈ `(-0.85, -0.55)`) rather than being a number quoted from a local run.
