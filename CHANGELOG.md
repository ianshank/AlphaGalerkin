...
- **gitleaks actually runs** — `.gitleaks.toml` and a `make gitleaks` target had both existed for months while **nothing invoked either**, in CI or pre-commit. A secret scanner that never runs is worse than none, because its presence in the tree reads as coverage. Now a named `Secret scan (gitleaks)` step in `ci.yml`; the Makefile target degrades *loudly* (it prints that the scan did NOT run and names CI as the enforcing copy) rather than passing quietly on a machine without the binary, since it is now chained into `make pre-pr`.
