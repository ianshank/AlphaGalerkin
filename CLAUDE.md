...
| **GPU headline run** | ✅ **VERIFIED on RTX 5060 Ti (sm_120, CUDA 12.8)**: `mse_low = 0.01551`, `mse_high = 0.01550`, `transfer_ratio = 0.999`, all PASS at YAML defaults (k=π, 200 epochs, 4096 colloc, d_model=64). Wall time 426s. | Verified during PR #58 + PR #61 + post-merge smoke. CI still runs the CPU smoke test only; the headline GPU run remains a hardware-gated manual reviewer step. |
