...
- [2026-08-14]: **Code Hygiene & Modularity Audit + Quick Wins** — `docs/CODE_HYGIENE_AUDIT.md` (excluded from the MkDocs site) audits `src/`/`tests/`/CI for god modules, duplicated scenario boilerplate, layering drift, and enforcement gaps, and documents 16 backlog items (god-module splits, registry/config/logging consolidation onto `src/templates/`, the `poc`↔`research` cycle, mypy gate determinism, hf_space single-sourcing, etc.). Landed 8 low-risk quick wins in the same PR: `mypy src/ --strict` is now genuinely clean (the measured gap was 3 stale `unused-ignore` comments, not the error mountain the initial audits assumed); ruff scope now matches between CI and pre-commit in both directions (`RUF100` catches stale `noqa`s going forward); the dead `benchmark` CI job (0 matching tests) is removed and `--strict-markers` is on; a shared `derive_seeds` helper in `src/seeding.py` replaces 6 duplicated seed-derivation bodies without changing any scenario's stride value (unifying 1009 vs 7919 is deliberately deferred — it would invalidate committed baselines); `tests/poc/conftest.py` adds a save/restore fixture around the `ScenarioRegistry` singleton (additive; the full local-fixture prune is backlog); the 3 classic PoC scenarios route through `resolve_device`; 2 dead constants in `src/constants.py` were deleted and 2 more wired to their live `src/modeling/` twins; 4 silent exception swallows now log. A peer review of the initial plan corrected several judgments before implementation (e.g. the mypy magnitude, and two "obviously safe" deletions — hydra-addressable config YAMLs and a full registry-fixture prune — that turned out not to be); see the audit's §6 for each reversal.
