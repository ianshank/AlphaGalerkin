...
- [2026-02-01]: ~~**Video Compression Hyperprior Fixed** - Proper z_bitstream encoding/decoding for entropy model~~ **[REMOVED 2026-07-22 — cut to the core; `src/video_compression/` no longer exists]**
...
- [2026-08-14]: **Code Hygiene & Modularity Audit + Quick Wins** — `docs/CODE_HYGIENE_AUDIT.md` (excluded from the MkDocs site) audits `src/`/`tests/`/CI for god modules, duplicated scenario boilerplate, layering drift, and enforcement gaps, and documents 16 backlog items (god-module splits, registry/config/logging consolidation onto `src/templates/`, the `poc`↔`research` cycle, mypy gate determinism, hf_space single-sourcing, etc.). Landed 8 low-risk quick wins in the same PR: `mypy src/ --strict --ignore-missing-imports` (the exact CI invocation) is now genuinely clean — the measured gap was 3 stale `unused-ignore` comments, not the error mountain the initial audits assumed. Note the flag is load-bearing: *bare* `mypy src/ --strict` still reports 10 `import-not-found`/`import-untyped` errors for optional deps (openai, skfem, pyamg, PicoGK, pettingzoo, gymnasium, pandas) that are not installed in the lint env; ruff scope now matches between CI and pre-commit in both directions (`RUF100` catches stale `noqa`s going forward); the dead `benchmark` CI job (0 matching tests) is removed and `--strict-markers` is on; a shared `derive_seeds` helper in `src/seeding.py` replaces 5 duplicated seed-derivation bodies without changing any scenario's stride value (unifying 1009 vs 7919 is deliberately deferred — it would invalidate committed baselines); `tests/poc/conftest.py` adds a save/restore fixture around the **structlog** global config (**corrected 2026-08-15**: this bullet originally also claimed a `ScenarioRegistry` snapshot/restore fixture — that half was attempted and **reverted before merge** as a measured net regression; see the audit's §6 and B16); the 3 classic PoC scenarios route through `resolve_device`; 2 dead constants in `src/constants.py` were deleted and 2 more wired to their live `src/modeling/` twins; 4 silent exception swallows now log. A peer review of the initial plan corrected several judgments before implementation (e.g. the mypy magnitude, and two "obviously safe" deletions — hydra-addressable config YAMLs and a full registry-fixture prune — that turned out not to be). CI itself caught two further mistakes within minutes of the PR opening — a config YAML deleted on a grep that couldn't see a test's f-string-constructed path, and `stochastic_galerkin_compare_config.py` adopting `derive_seeds` in violation of that layer's curated import-isolation allowlist — both fixed by reverting to the original, narrower behavior rather than widening a guard; see the audit's §6 for these and every other reversal.
- [2026-08-14]: **Tech-Debt Phase 1 — CI Enforcement + Hardcoded-Value Surfacing** — consumed the hygiene audit's backlog under a five-agent adversarial peer review (which killed one CI step that would have gone red on merge and corrected two workstream designs before implementation). **(1) CI actually enforces now**: `on.pull_request` no longer filters on the nonexistent `main`/`develop` branches — CI had *never* run on any PR; `test-slow` keys on `github.event.repository.default_branch`; the dead `.pytest_cache/` upload step and cache-defeating `--no-cache-dir` are gone. **(2) Four broken coverage gates fixed (B20 closed-wider)**: three CLAUDE.md-documented gates never existed in `ci.yml` (`noyron_basis` 98% measured, Noyron HX surface 99%, SBIR P40 surface 94% — all wired native-runner at 85) and the llm_prior gate was *silently degraded* (coverage 7.x drops file-path `--cov=…py` specs with only a warning; the file pair decayed unenforced to a measured 77%/100% — repaired native-runner, gated 79 = measured−2, ratchet tracked). A CAUTION comment in `ci.yml` bans the file-spec form. **(3) Unenforced guarded surface re-enabled**: `tests/pde/test_mcts_adapter.py` (documented F1/F3 surface) had run in NO CI job since the 2026-04 triage; re-enabled green along with 2 CUDA deselects redundant with in-source `skipif`s — the remaining 9 deselects + 6 ignores all pass at HEAD (stale triage, not broken tests; staged Phase-2 deletions). **(4) Hardcoded values surfaced at zero numeric change** (asserted by value-identity checks): LR-scheduler knobs reconciled across their *three* divergent copies (`TrainingConfig.min_lr_ratio`/`warmup_start_factor` = 0.1/0.1 typed fields; `DEFAULT_MIN_LR_RATIO`/`DEFAULT_WARMUP_START_FACTOR` = 0.01/1e-6 bind `BaseTrainerConfig` + `_create_scheduler`); boundary tolerances named not unified (`DEFAULT_PICOGK_BOUNDARY_TOLERANCE` 1e-5 documents the SDF-band semantic; `is_boundary_point` has zero production callers); checkpoint-migration defaults intentionally frozen with a drift-alarm test; Gumbel epsilons split by semantic (`GUMBEL_NORMALIZATION_EPSILON` inert vs `GUMBEL_LOG_PRIOR_FLOOR` algorithmic ≈−18.4 score); `FNetEvaluator._SOFTMAX_NORMALIZER_FLOOR` mirrors `lm_studio/evaluator.py` by name; 13 `[9,13,19]` sites → `list(DEFAULT_BOARD_SIZES)` copies. **(5) Dead `BaseTrainer` abstraction demoted**: the 3 `@abstractmethod`s both production trainers stubbed with `NotImplementedError` are now concrete `step()`-loop hooks (subclass stubs kept — message-asserted); `audit_abstractions` baselines recorded (`src/training`: `BaseLoss.forward`; `src/pde`: `PDEGame.get_result` = B17, resolution decided demote+hoist). **(6) Measured, not guessed**: branch-coverage actuals recorded for 8 ungated packages (agents 95.4 → math_kernel 61.5; gates land Phase 2c at actual−2) and the mypy override debt quantified at 207 masked errors across 51 modules. B10 reclassified: only `prototyping`/`tournament`/`analysis`/`curriculum` are dead — `deployment` is CI-exercised and `demos` is a live dashboard dependency. Roadmap + owner-decision register in `docs/CODE_HYGIENE_AUDIT.md` §7.
- [2026-08-15]: **Dead-Abstraction Removal (B17) + Abstraction Audit Gated in CI (B18)** — follow-up to the hygiene audit, closing its two deferred backlog items. (1) **`PDEGame.get_result` and `PDEResult` deleted.** `get_result` was `@abstractmethod`, documented as lifecycle step 4, implemented by all three concrete games — and never called (the `get_result` call sites in `src/training/evaluation.py`/`src/engines/match.py` are the unrelated 1-arg `GameInterface` form). **Deleted rather than wired**, on evidence: all five real episode-terminal paths already build their own result object and each needs a field `PDEResult` lacks (`actions`, `solution`/`wall_time_seconds`, `rollouts_used`, `n_solves`), while six of its seventeen fields had no reader anywhere in `src/`; everything real was already reachable via `compute_exact_error` (which *does* have a call site) and `PDEGameAdapter.error_history`. Contrast F1's `get_reward`, which was *wired* because MCTS consumes its semantics — `get_result` changes no algorithm and no committed number. (2) **`PDEGame.termination_reason` added** — the cause ladder that was inlined in all three deleted overrides, promoted to the ABC with the one game-specific rung behind a `_capacity_reason` hook (basis count; DOF with `>` for mesh_refinement, `>=` for lshape_amr, each mirroring its own `is_terminal`). **Concrete, not abstract**, so no subclass breaks. (3) **It has a real consumer**: `AlphaGalerkinSolver` records it under `METADATA_KEY_TERMINATION_REASON`, which previously read the bare `"is_terminal"` — *breaking for consumers string-matching that value*. `SolverResult.h1_error` is populated in the same call site (the field and its serialisation existed; the solver just never set it, exporting a permanently-null column). (4) **CI gates the F0/F1 screen**: the `lint` job runs `audit_abstractions src/mcts src/refinement src/pde --fail-on-missing` (AST-only, stdlib imports, so it fits that job's minimal dep set) plus a report-only pass over all of `src/` — the `src/backend` domain-PoC backlog stays advisory, and promoting that step to blocking is what remains. Also corrected the never-executed `MCTS->>PDEGame: get_result(...)` flow in `docs/architecture/pde_game_c4.md` (those terminal calls belong to the solver). Evidence and the full WIRE-vs-DELETE analysis: `docs/CODE_HYGIENE_AUDIT.md` §4.1.
- [2026-08-16]: **L-shape AMR Convergence Defect — Retraction + Convergence Gate** — peer review flagged that neither AMR arm converged; investigation found the cause is a **boundary-condition defect**, not the marking strategy. `lshape_inside_predicate` removed the **open** quadrant `x>0 and y<0`, but the L-shaped domain `[-1,1]² \ [0,1]×[-1,0]` has the two reentrant edges `{y=0, x≥0}` and `{x=0, y≤0}` **on its boundary**, where `u = r^(2/3)sin(2θ/3)` is identically zero. Strict inequalities made every slit-edge node an interior unknown, so its `u=0` Dirichlet condition was never imposed and the stencil coupled across the slit into the analytic continuation pinned inside the notch — a different, inconsistent problem. **Diagnosed with no marking involved**: under *uniform* refinement the L2 error **grew** with DOF (5.0e-2 at 65 DOF → 1.15e-1 at 12545 DOF, rate −0.09); peak error sat on the slit edge at `(0.75, 0.0)` far from the corner, growing 0.357 → 0.519 → 0.634 as h halved. Removing the **closed** quadrant (`>=`/`<=`, one line) restores the textbook rate for the reentrant-corner singularity: measured **O(h^1.31)** ≈ O(N^-0.65) ≈ O(N^-2/3), taking the same n=128 grid resolution to 2.59e-4 at 12416 DOF (12545 DOF pre-fix — the fix pins 129 more nodes as Dirichlet) — **444× lower**. **(1) Headline retracted**: re-running the canonical 5-seed demo config on the fixed substrate flips the result — `l2_error_ratio_at_matched_dof` **0.9605 → 1.0996** (MCTS now *loses* ~10%), `mcts_win_fraction` 0.80 → **0.20**, `l2_error_ratio_at_matched_solves` 1.26 → **2.04** (0/5 seeds), Dörfler final L2 @ ~1300 DOF 9.04e-2 → **8.40e-3**. The primary threshold now *fails*, which is the falsifiable gate working as designed. **(2) Second defect unmasked**: with the BC fixed, adaptive Dörfler converges at only −0.125 while *uniform* refinement on the identical substrate achieves −0.65 — adaptive marking is **5–9× worse than uniform at matched DOF**, gap widening (9.1× at ~1300–1800 DOF). Cause is the tensor-product refinement already recorded as a scope note: `_dorfler_mark_2d` projects element marks onto the x/y axes and `_refine_grid` runs separately on `xs`/`ys`, so one marked element inserts full grid *lines* across the whole domain. This makes element-local refinement (v2.1, skfem/quadtree) a **blocking prerequisite** for any marking-policy comparison, not an optional upgrade. **(3) Convergence gate**: new `tests/research/test_lshape_convergence_gate.py` (11 tests, 1.2 s) asserts monotone error reduction under uniform refinement, the O(h^4/3) rate band, an absolute finest-grid anchor, reentrant-edge pinning, and that the *Dörfler arm alone* reduces error with DOF — mutation-tested (7 of 11 fail on the pre-fix predicate). `LShapedDomain.contains_point` is deliberately **unchanged**: closed-domain *membership* and interior-*unknown* selection are different questions, and conflating them was the bug. Docs corrected repo-wide (charter claims register, `specs/lshape_amr_compare.spec.md`, CHANGELOG); regenerated `results/lshape_mcts_vs_dorfler.{csv,png}`.
