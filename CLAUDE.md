...
- [2026-06-20]: **LLM-Eval Harness Integration (`langfuse-eval-harness`)** — additive `src/integrations/eval_harness/` adapter onto the external `github.com/ianshank/Agents` framework (package `langfuse-eval-harness`, import root `eval_harness`, SHA-pinned `1dd95ff`; **not on PyPI** → VCS dep). Traces + labelled-scores the LLM-prior MCTS basis-selection layer. The harness built-in `callable` target runs `target.run_basis_cell` (reusing `_centaur_common` + `PDEGameAdapter` verbatim) emitting `final_residual`/`rollouts_used` + a root-state `chosen_action`/`topk_actions`; `oracle.greedy_basis_oracle` produces the exact-but-**myopic** 1-step greedy label via `BasisSelectionGame.apply_action`; `scorers.{FinalResidualScorer,PolicyTopKScorer}` + `dataset.BasisOracleDataset` (+ `build_dataset_jsonl` cache) plug into the harness `SCORERS`/`SINKS`/`DATASETS` registries through an idempotent **torch-free** `register_all` (and an `eval_harness.plugins` entry-point `_entrypoint` shim, so global `bootstrap()` never pulls torch). The single authoritative cross-run gate stays `src/poc/baselines`: `sink.ScenarioResultSink` writes a `ScenarioResult`-shaped JSON into `{output_dir}/results/{run_id}/` so the existing `record-baseline`/`diff` CLI gates harness runs (new `_pass_rate` higher-better suffix); the harness `GateConfig` is an independent in-run sanity check. Langfuse is opt-in — CI runs `--offline` (dependency-free `NullLangfuseClient`); the live `SDKLangfuseClient` is the *separate* `[eval-harness-langfuse]` extra pinned `langfuse>=2,<3` (the harness's `SDKLangfuseClient` calls the v2 score API while a bare `langfuse>=2` resolves the OTEL v3/v4 SDK). New `python -m src.poc.cli eval-harness --config … [--online] [--baseline …]` subcommand; new YAML `config/eval_harness/basis_eval.yaml`. New extras `[eval-harness]` (SHA-pinned VCS dep) + `[eval-harness-langfuse]`; mypy `ignore_missing_imports` for `eval_harness.*`/`langfuse.*`. **Coverage**: torch-free modules (`config`/`scorers`/`sink`/`plugins`/`_import_harness`/`_entrypoint`/`__init__`) 100% line+branch; `oracle`/`target`/`dataset`/`runner` covered by `importorskip` torch tests; `test_contract.py` pins the upstream API. `ruff` + `ruff format` clean; `mypy --strict` (pinned 1.10–1.12) zero issues on the adapter.
