...
| Codec perf benchmark (Phase 0) | `pytest tests/video_compression/perf/ -v` | Pydantic config validation, percentile/throughput math, baseline registry forward-compat (unknown-field tolerance + unversioned-file migration), per-cell device pinning (`cuda:N`), regression diff (throughput / latency-p50 / latency-p99 / VRAM with per-entry tolerance overrides), `BenchmarkPhase` factory dispatch, CLI `run` / `record-baseline` / `diff` exit codes, GPU smoke on `cuda:0` and dual-GPU sweep on `cuda:0`+`cuda:1` (auto-skipped via `gpu_required` marker when CUDA is unavailable) |
