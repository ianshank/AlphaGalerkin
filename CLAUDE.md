...
| **Missing per-package `AGENT.md` files** | `src/refinement/AGENT.md` and `src/alphagalerkin/AGENT.md` do not exist, though root `AGENT.md` says every module has one | Both packages are **core** maturity per `ARCHITECTURE.md`; write the two files following the existing per-module template (see `src/pde/AGENT.md`) |
