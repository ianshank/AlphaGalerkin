...
| Abstraction audit (F0/F1 screen) | `python -m scripts.audit_abstractions src/mcts --fail-on-missing && python -m scripts.audit_abstractions src/pde` | Every `@abstractmethod` has a call site and every `Protocol` member a reader on the refinement surfaces. `src/mcts` and `src/refinement` are clean and gated with `--fail-on-missing` (`get_reward`/`n_players` now have call sites). **`src/pde` is deliberately run *without* `--fail-on-missing`: it has one known dead abstraction, `PDEGame.get_result` (`src/pde/game.py:457`) — declared abstract and implemented by every concrete game, but never called (the `get_result` calls elsewhere are the unrelated 1-arg `GameInterface` form). Treat any *additional* hit as a blocker; resolving the baseline hit is `docs/CODE_HYGIENE_AUDIT.md` B17.** Domain-PoC backlog (`backend`) is non-blocking report-only. |
