...
- [ ] Wire the substrate into a `RefinementGame` and retire the two-path period (`openspec/changes/element-local-substrate/` Slice E) — the substrate exists and is gated, but nothing in a non-test process constructs one yet, so `RefinementSubstrateRegistry` has two runtime registrants but zero runtime *lookups* -- nothing resolves a substrate by key yet (a disclosed charter deviation, narrowed 2026-09-02). This is the prerequisite for the MCTS-vs-classical arena, which is deliberately a *separate* change
