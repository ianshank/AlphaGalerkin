...
| B39 | **`basis.get_dofs()` is called three times per scikit-fem solve** on the path the spec names as MCTS's dominant cost: `fem_baseline.py` inside `dirichlet_dof_indices`, again in `assemble_and_solve` two lines before it *calls* `dirichlet_dof_indices`, and a third time in `SkfemTriSubstrate.solve` for `n_dof_free` on the same basis. The `basis=` parameter was added to `assemble_and_solve` precisely to avoid a *third assembly*; the DOF query was not given the same treatment | S | Low | Return the Dirichlet indices from `assemble_and_solve` (or accept them) so the substrate reuses them. Changes a primitive's return signature under byte-identical goldens — its own change, not a hygiene edit |
| B40 | **Pre-existing `fem_baseline.py` gaps the branch-coverage audit surfaced but that predate this branch**: the entire `p_adaptive` / `hp_adaptive` refinement-strategy block (8 arcs, 17 statements, reachable via `AMRConfig.refinement_strategy`), `_make_element`'s unknown-element raise, and `_order_of` (dead only because those paths are its sole callers). The file clears its gate at 86 vs 83 with all of this unmeasured | M | Low | One parametrised test over `refinement_strategy in {"p_adaptive", "hp_adaptive"}` reaches most of it; not this branch's code, so not this branch's pass |
