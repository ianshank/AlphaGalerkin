...
| `src/research/baselines.py` | **1540** (was 1460; now the largest file in `src/` — see B34) | Config schemas + solver implementations + registry in one file; the sibling `src/research/extra_solvers/*.py` already demonstrates the intended split-file + `SOLVER_REGISTRY` pattern |
...
| B34 | **`src/research/baselines.py` is now the largest file in the repo at 1540 lines** (overtaking `training/trainer.py` at 1539), and the element-local-substrate work *added* to it — `nodal_rms_l2_error`, `element_inside_mask`, `require_exact_solution`. Those extractions were right (they each replaced a copy-paste between the two substrates), but the destination is a god file getting bigger. It is named in §3.1's B4 list and still untouched | L | High | Deliberately **not** split on the element-local-substrate PR: `baselines.py` is imported by the SBIR P40 surface, both substrates, the L-shape harness and the AMR baselines, and that PR is already 5k additions with a CI run that had never completed. Splitting it belongs in its own PR with the B21 recipe (import-compatible `__init__` re-exports, public-API guard test, byte-identical `mypy --strict` error set) |
