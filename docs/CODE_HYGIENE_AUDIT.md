...
| ~~B2~~ ✅ **DONE (2026-09-10)** | `CompareScenarioBase` + name-lock function across the four `*_compare` families (`_compare_lock.py` torch-free for configs; `_compare_common.py` for teardown / metrics protocol / CSV-PNG write). Arena keeps `write_arena_manifest`. Config→Params builders stay per-family (not a shared Params type). | L | Med | Four scenario surfaces in one pytest process (161 passed). `compute_hash()` pinned for the three datetime-free configs. |
...
| B7 | CI composite action for the ×10 checkout/setup/install preamble + a shared `--ignore`/`--deselect` args file (currently duplicated verbatim between `test-fast` and `coverage`). **The third sub-item, `COVERAGE_CORE` centralization, is closed 2026-09-02 by deletion rather than centralization** — there is nothing left to centralize, see B36. **[2026-09-09: ledger half]** owner / reason / reopen criteria for the duplicated list now live in `docs/ci-exclusion-ledger.md`, guarded by `tests/docs/test_ci_exclusion_ledger.py` (the three copies must match the YAML fence). The args-file extract itself is still deferred | M | Low-Med | Preserve step names — the charter's Quality-Gate guard parses `ci.yml` by `- name:` |
| ~~B8~~ ✅ **DONE (2026-09-10)** | CLAUDE.md Regression Surface coverage-gate rows ⊆ CI (`tests/docs/test_claude_coverage_gates.py`). Filter is `--cov-fail-under` / native `--fail-under` only — whole-table ⊆ fails on GPU-smoke rows. Mutation-killed with a planted `src/planted_b8` / 99 gate. | M | Med | New test under `tests/docs/`; don't conflict with the existing charter meta-guards |
...
| ~~B12~~ ✅ **DONE (2026-09-10)** | AGENT.md for every `src/` package, starting at `src/research`, plus four B10 keep-reason files. Guard: `tests/docs/test_package_agent_md.py`. Historical 14-of-28 count is retained in CLAUDE.md Next Steps as a resolved row. | M | None | `check_doc_links.py`. **[2026-09-02: re-counted — **15 of 28**, not 14 of 26; the old figure predated `src/core` and `src/refinement`. `src/refinement/AGENT.md` written.]** |
...
| ~~B31~~ ✅ **DONE (2026-09-02)** | **`src/backend` was invisible to every coverage gate in the repo** — `src/backend/*` sits in pyproject.toml's global `[tool.coverage.run] omit` (legitimate origin: `jax_backend.py` needs the optional `[jax]` extra) **and** had no per-module gate, so its 213 passing tests measured nothing anywhere. Real branch coverage is **56%**, and the omit was hiding `logging.py` and `rng.py` at literally **0%**, not just the JAX module it was written for. This is the **fourth** instance of the omit-collision false pass in this repo (after `video_compression`, `demos`, `skfem_tri.py`) and the first that `tests/docs/test_coverage_gate_integrity.py` could not have caught — see B32 | S | Low | New `Per-module coverage gate (backend)` step at **54** using the inline-coveragerc technique; `src/core` gated at 85 (measured 97.67%) and `src/deployment` at 25 (measured 27.91%) in the same pass |
| ~~B32~~ ✅ **DONE (2026-09-02)** | **The coverage-gate integrity guard checked only half the question.** It verified that a gate a workflow *declares* is not neutered by the omit; it said nothing about a package that is omitted **and never gated at all** — not a fake gate, but no gate, invisible for the same reason. B31 sat in that blind spot and was found by hand. Added the missing direction: every `omit` entry must be gated by a step that both selects it *and* overrides the omit, or carry an entry in `_OMIT_WITHOUT_A_CI_GATE` with a reason. Self-expiring in both directions — a stale exemption fails once a gate appears, and an exemption for a pattern no longer in `omit` fails as vacuous | S | Low | `tests/docs/test_coverage_gate_integrity.py` (20 tests); 2/2 mutation-killed (dropped `--cov-config` on the new backend gate; an exemption naming an already-gated pattern) |
| ~~B33~~ ✅ **DONE (2026-09-02)** | **`docker/Dockerfile` shipped 2026-08-16 and was built by nothing** — no CI job, no Makefile target, no test — while CLAUDE.md's Next Steps simultaneously recorded, "verified 2026-08-21", that no Dockerfile existed anywhere in the tree. The verification was wrong five days after the fact and nothing caught it for two weeks. Verified by hand that the image's `CMD` currently works by materialising the real build context (git-tracked files minus `.dockerignore`) and running it: 510 passed, 6 skipped | S | Low | `tests/docs/test_dockerfile_context.py` — **hermetic, no daemon**: every `COPY` source must survive `.dockerignore`, every copied path must exist, and the `CMD`'s trees must be reachable. Plus `make docker-build` / `make docker-test` with `DOCKER_IMAGE`/`DOCKER_CONTEXT`/`DOCKERFILE` as overridable variables. 2/2 mutation-killed |
| ~~B35~~ ✅ **DONE (2026-09-11, R-02)** | **Closed by R-02** — the fast lane is now **hermetic**: `test-fast` and `coverage` run under `pytest-socket` (`--disable-socket --allow-unix-socket --allow-hosts=127.0.0.1,localhost`, the workflow-level `HERMETIC_PYTEST_FLAGS`) with a registered `network` marker deselected by `-m`; the two VGG tests carry `@pytest.mark.network` (via the `offline_vgg` treatment in `tests/video_compression/unit/test_loss.py`) and are excluded from the lane rather than mocked. The Makefile's `test-fast` / `coverage` mirror the flags. `tests/docs/test_fast_lane_is_hermetic.py` keeps the three copies equal and *drives* a planted outbound connect to prove the block is real (4/4 mutation-killed). Original row follows. **Two "unit" tests download 500 MB of ImageNet weights over the network.** `tests/video_compression/unit/test_loss.py::TestCompressionLoss::{test_perceptual_loss_enabled,test_total_combines_all_losses}` reach `src/video_compression/metrics/quality.py:406`'s `_get_vgg`, which calls `vgg16(weights=VGG16_Weights.IMAGENET1K_V1)` and fetches from `download.pytorch.org`. They **fail** in any sandboxed or air-gapped environment (observed 2026-09-02: `OSError: Tunnel connection failed: 403 Forbidden`) and pass on GitHub's networked runners, which is why CI has never flagged them — `tests/video_compression/` is *not* in `CI_TEST_EXCLUDES`, so CI runs and silently depends on egress. Same accident-of-environment class as `test_path_traversal_in_config` and the checkpoint test this repo already fixed once | S | Low | Mock `_get_vgg`, or mark the pair `@pytest.mark.network` and deselect it from the fast lane. **Deliberately not fixed on the element-local-substrate PR**: an unrelated subsystem, and `scripts/check_focus.py` exists precisely to stop that kind of widening. 9600/9602 of `make test-fast` pass; these two are the whole delta |
| ~~B36~~ ✅ **DONE (2026-09-02)** | **A test-enforced, ~40-document-wide invariant was false.** `COVERAGE_CORE=pytrace` was set in 5 places (two `ci.yml` jobs, two `test-extras` steps, a third job that runs no coverage at all, `Makefile`, `.claude/settings.json`), asserted by `tests/claude/test_harness_validation.py`, and justified across ~40 docs by two commits: `cb645ad` (2026-07-10) said the installed torch wheel crashed coverage's default C tracer at collection; `cfe7f22` (2026-08-15) said it silently UNDER-measured (`src/training` at 89.53% pytrace vs 82.45% C, `base_trainer.py` at 46%). **Neither reproduces.** A minimal direct reproduction of the documented crash (`coverage run --branch` over a script importing `torch._C` and running a tensor op) exits 0 under both cores; the identical `src/training` gate measures **88.25% under both**, byte-identical per file. CI's own logs then confirmed it at scale. The two claims were also mutually inconsistent — a crash at collection and a silent wrong number are different failures — which is the tell nobody followed up on for two months. Note the cost was never hypothetical: the pin was making every coverage job ~3× slower, which is what pushed the combined `coverage` job past its 45-minute cap (see the CI-timeout split) | S | Low | Removing the pin from the `coverage` job cut pytest execution **1967.30s → 604.32s (3.26×)** with byte-identical totals (`TOTAL 29971 2232 7150 91%`, 90.70%), zero crash signatures, and all 43 per-module gates green. `test_coverage_core_is_pinned_to_pytrace` inverted to `test_no_coverage_core_tracer_pin` (asserts absence; mutation-killed by reintroducing the key). Closes B7's third sub-item by deletion |
| ~~B37~~ ✅ **DONE (2026-09-11, R-13)** | **Closed by R-13** — every file under `tests/integrations/eval_harness/` now carries a registered `eval_harness_required` marker (the module-level `importorskip` that collapsed eight files to zero items is gone; module-scope imports of the extra moved into fixtures, so all 39 items collect on a base install). The root `conftest.py` gates the marker through the same `_gate_optional_extra` body as `fem_required`: a counted skip normally, `pytest.UsageError` under `ALPHAGALERKIN_REQUIRE_EXTRAS=1`. `test-extras` selects `-m "eval_harness_required"` and gates `--cov=src/integrations/eval_harness` through an inline coveragerc that overrides the global `omit`, at `--cov-fail-under=96` — landed as a `1` tripwire (the extra could not be installed where it was authored); the first green-reporting `test-extras` run (34658949836, the same day) measured **98.16%** branch and exposed a real defect (`config/eval_harness/basis_eval.yaml` passed `function:` to a `CallableTarget` whose parameter is `path`, in a test that had been silently skipped since it was written); gate set to `floor(98)-2 = 96`. `_OMIT_WITHOUT_A_CI_GATE` is now empty. Guard: `tests/docs/test_eval_harness_gating.py` (7/7 mutation-killed). Original row follows. **`src/integrations/eval_harness` is 914 LOC measured by nothing, and the exemption saying otherwise was false.** The package is in pyproject.toml's global coverage `omit` and has **no** `--cov` gate in any workflow. `test-extras` installs `.[dev,test-extras,eval-harness]` (`ci.yml:1179`) with a comment saying it does so *specifically* to stop 8 of the 11 modules under `tests/integrations/eval_harness/` skipping at import — then runs none of them (measured: `11 passed, 8 skipped`). Worse, `tests/docs/test_coverage_gate_integrity.py`'s `_OMIT_WITHOUT_A_CI_GATE` entry justified the gap with two clauses that are both false: "which no CI job installs" (one does) and "enforced instead by the `Eval-harness adapter` Regression Surface command" (no such row exists in CLAUDE.md, and never did). The guard fired and was silenced with an untruth. Reason now rewritten to the truth and marked a **disclosed gap**, not a justified exemption | S | Low | The real fix is small — `test-extras` already has the extra installed, so a per-module gate there is a few lines. Meanwhile `test_regression_surface_rows_cited_by_exemptions_exist` closes the mechanically-decidable half of the hole: an exemption naming a backticked Regression Surface row must name one that exists. Mutation-killed by restoring the original citation |
| ~~B38~~ ✅ **DONE (2026-09-02)** | **Three hard-failing CI jobs cannot fail the merge gate.** `ci-success.needs` (`.github/workflows/ci.yml`) lists 6 of 13 jobs; `focus` and `secrets` carry explicit "deliberately and temporarily omitted" comments, but `test-integration`, `test-jax` and `test-chess` carry nothing — and `test-chess` runs its own `--cov-fail-under=80` on `src/games`. If branch protection requires only `CI Success`, a red Integration/JAX/Chess job is mergeable. This is the failure `ci.yml`'s own `ci-success` comment articulates ("A gate that cannot fail the build is a report, not a gate") three jobs earlier. Separately, every push fires both a `push` and a `pull_request` run into one concurrency group, and `cancel-in-progress` kills whichever registers first — so `focus` (PR-event only) and `test-slow`'s `[full-test]` hatch (push-event only) are each a coin flip per push | S | Med | Was deliberately deferred one pass (a CI-trigger edit on a just-green PR); then the race mis-led a human a **second** time within the hour, which settled it. Fixed with user sign-off on both halves: `push:` restricted to post-merge branches (default branch hardcoded — the risk `ci.yml`'s own comment warns about, named in a new comment; `workflow_dispatch` added to `test-slow` as the manual hatch), and all three jobs added to `ci-success.needs` after a YAML parse confirmed none has a job-level `if:` that could turn `skipped` into a false red. Expect **one** run per push from here. **Remainder closed 2026-09-11 (R-09):** the two "deliberately and temporarily omitted" jobs, `focus` and `secrets`, are now in `ci-success.needs` *and* its `exit 1` block. `focus` is `pull_request`-only, so it is gated by *shape* — one clause accepts `skipped` when the event is not a PR or the `focus-override` label is present, one rejects `skipped` on an unlabelled PR — and `secrets` shares `ci-success`'s `if:` and gets a bare check. The same change (R-12a) added the new `typecheck` job to the hard set. `tests/docs/test_ci_success_hard_gates.py` now requires **every** job in `ci.yml` to be a hard gate or a disclosed, self-expiring `DISCLOSED_NOT_HARD` entry, so this row's failure mode cannot recur silently (4/4 mutation-killed). |
| ~~B39~~ ✅ **DONE (2026-09-10)** | **`basis.get_dofs()` is called three times per scikit-fem solve** on the path the spec names as MCTS's dominant cost: `fem_baseline.py` inside `dirichlet_dof_indices`, again in `assemble_and_solve` two lines before it *calls* `dirichlet_dof_indices`, and a third time in `SkfemTriSubstrate.solve` for `n_dof_free` on the same basis. The `basis=` parameter was added to `assemble_and_solve` precisely to avoid a *third assembly*; the DOF query was not given the same treatment. **Fix:** `dirichlet_dof_indices(..., dofs=)` threads the raw Dof; `assemble_and_solve` still calls `basis.get_dofs()` once (needed for `condense(D=)`) and returns the flattened indices as a fourth value. The solver wrapper stays a 3-tuple. `SkfemTriSubstrate.solve` uses the returned indices. | S | Low | `TestAssembleAndSolveSharesDofQuery` spies one `get_dofs`; `test_precomputed_dofs_skip_get_dofs` mutation-kills the skip |
| ~~B40~~ ✅ **DONE (2026-09-10)** | **Pre-existing `fem_baseline.py` gaps the branch-coverage audit surfaced but that predate this branch**: the entire `p_adaptive` / `hp_adaptive` refinement-strategy block (8 arcs, 17 statements, reachable via `AMRConfig.refinement_strategy`), `_make_element`'s unknown-element raise, and `_order_of` (dead only because those paths are its sole callers). The file clears its gate at 86 vs 83 with all of this unmeasured. **Fix:** parametrized strategy test plus P3 saturation, hp h-refine when smoothness is below threshold, unknown-element raise. Gate stays 83 (arcs covered; threshold not ratcheted). | M | Low | `tests/research/test_fem_baseline.py::TestPAndHpAdaptiveStrategies` / `TestMakeElementUnknown` |
| ~~B34~~ ✅ **DONE (2026-09-10)** | **`src/research/baselines.py` was the largest file in the repo** (1578 lines). Split into `src/research/baselines/` with import-compatible `__init__` re-exports, public-API freeze (`tests/research/test_baselines.py::TestBaselinesPackagePublicAPI`, not raw `dir()` identity), `pyproject.toml` mypy glob `src.research.baselines.*`, SBIR `--include` listing both `*/src/research/baselines.py` (package `__init__.py`) and `*/src/research/baselines/*`, and the reference-baselines import contract scoped to the package directory. `extra_solvers` still register into `SOLVER_REGISTRY`; this package does not import them. | L | High | B21 recipe; SBIR P40 / substrate / L-shape surfaces stay green together |
| ~~B19~~ ✅ **DONE (2026-09-10)** | Documented extras `fem`, `viz`, `dev`, `test-extras`, `jax` / `jax-gpu`, `picogk`, `lm-studio`, `docs` in README / getting-started / CONTRIBUTING. **No `dashboard` extra** (FOCUS-frozen interactive-surfaces). | S | Low | None (packaging metadata + prose) |
...
| ~~B22~~ ✅ **DONE (2026-09-11)** | **Concurrent-subagent working-tree safety.** Dispatching 3 background agents against non-overlapping file sets in the same working tree (rather than isolated git worktrees) hit a real collision during PR #140: one agent's own `git stash`/`git reset` (used internally, not instructed) stashed away a second agent's uncommitted, unrelated test-file edits alongside its own in-progress work. No data was lost this time — HEAD and prior commits were unaffected (`git reset` with no target is index-only), and the second agent recovered its own files via `git checkout stash@{0} -- <its files>` — but the mechanism is a real hazard: any subagent with Bash access can run working-tree-wide git commands that affect every other concurrently-running agent, whether or not their file scopes overlap. Options: default to `isolation: "worktree"` for any multi-agent dispatch that gives subagents Bash+git access, or add an explicit instruction in dispatched prompts restricting subagents to `git status`/`git diff`/`git add <pathspec>` and forbidding `git stash`/`git reset`/`git checkout` without a narrow pathspec. | S | Low | Process/orchestration convention, not a code change; worth a line in `AGENT.md` or the relevant `.claude/agents/*.md` frontmatter for agents commonly run concurrently. **[2026-09-11: DONE — reflection ticket R-14.]** The second option, made checkable: root `AGENT.md` gained a `## Concurrent subagents` section (one anchor sentence — *"Concurrent subagents work in their own `git worktree` and never run `git stash`, `git reset`, `git checkout -- <path>` or `git clean` in a shared working tree."* — plus four clauses: sibling worktree per subagent via the already-allowed `Bash(git worktree:*)`, no tree-wide git commands in a shared tree, commit on a dedicated branch and hand the name back, and run `python -m …` from the worktree root until the R-04b lockfile retires the editable-install trap) and retells this incident. The anchor is repeated **verbatim** in all six `.claude/agents/*.md` — every one declares `Bash`, and a rule stated only in a file the subagent never opens is stated nowhere. Guard: `tests/claude/test_worktree_rule.py` (`TestConcurrencyRule` / `TestWorktreePermission`, 18 tests) — anchor exactly once in root `AGENT.md`; the test's constant read back from the doc so neither can drift alone; every Bash-declaring agent (discovered on disk through `test_harness_validation.py`'s parser, not listed) carries the anchor and clause 4; non-vacuity (≥ 1 Bash agent); `settings.json` actually grants what the doc claims. **5/5 mutation-killed of six planted**: anchor deleted from one agent, one word altered in the doc's copy, `Bash` stripped from *every* agent (the vacuity guard fires — pytest otherwise *skips* the parametrised tests as an empty set), the permission removed, clause 4 removed from one agent; the sixth — `Bash` stripped from one agent with its sentence kept — survives **by design** and is recorded as such: the rule is one-directional (Bash ⇒ anchor), since an agent that cannot run git carries surplus prose, not a false statement. No skill, agent or command was added (the CLAUDE.md inventory counts are unchanged). |
| ~~B23~~ ✅ **DONE (2026-09-10)** | **PR and CI schedule hygiene.** Two items observed while driving PR #140 to green. (2) **DONE (2026-09-01)**: a monthly Routine now lists open PRs and flags ones whose base SHA is 20+ commits behind their base branch's tip or with no activity in 30+ days (reporting-only by design — it does not comment/label/close, a human decides what to do with a flagged PR). One residual uncertainty: `create_trigger` warned that self-bound routines "store no MCP connectors," which if it turns out to matter in practice would mean the fired session lacks working GitHub tools when it runs next month — self-binding to the session that already holds this repo's GitHub MCP access was the mitigation, but this is unverified until the first firing. (1) **DONE (2026-09-10)**: every non-`test-slow` job in `.github/workflows/ci.yml` skips `schedule` (`if: github.event_name != 'schedule'`, or pull_request-only for `focus`; `ci-success` is `github.event_name != 'schedule' && !cancelled()`). `test-slow` still runs on schedule via `!cancelled()` so a skipped `test-fast` does not retire the cron. Guard: `tests/docs/test_nightly_schedule.py` (substring skip plus `||` is rejected — an OR would still fire on cron). The previously measured ~53-58 min duplicate nightly matrix is the defect this lands, not a remaining TODO. | S | Low | `pytest tests/docs/test_nightly_schedule.py -v` |
| ~~B24~~ ✅ **DONE (2026-09-01)** | **Fail-fast validation for `BasisSelectionGame` operators with no exact solution** (`id_pde`/`PDEName`/`ResearchPDEName`), landed via a **different mechanism than originally proposed here**. The graceful-arm-skip pattern this row originally suggested (mirroring the LLM-preflight-failure skip) turned out to be the wrong shape on investigation: `ExactSolutionUnavailableError` is raised inside `_centaur_common.run_basis_selection_cell`'s `PDEGameAdapter(game)` construction, and that function's `CellOutcome` return value is unpacked positionally (`rollouts_used, final_residual = ...`) at every one of its 5 call sites (`llm_prior_ablation.py`, `scaling_law.py`, `agents/research_loop.py`, `noyron_basis.py`, `src/integrations/eval_harness/target.py`) — adding a third `skipped` field would break that unpacking everywhere, and representing "skipped" via a sentinel inside the existing 2 fields (e.g. `final_residual=nan`) risks a NaN silently entering a median/aggregate calculation and being reported as a real metric, which is a *worse* defect than the current loud crash (it would be exactly the class of silent-degenerate-metric bug P0-1 itself exists to eliminate). Implemented instead: a `@field_validator` on each of the three config classes rejecting `heat`/`advection_diffusion` at **construction time** with a message naming `ExactSolutionUnavailableError` and this row, so the user gets an immediate, actionable error instead of a crash minutes into a run after other arms/seeds already spent compute — arguably better UX than either the original crash or a graceful mid-run skip. The rejection list (`_PDES_WITHOUT_EXACT_SOLUTION`, one frozenset per file) is duplicated across the three files rather than imported from a shared module, matching `agents/config.py`'s own pre-existing, documented decision to stay decoupled from the heavy MCTS/PDE import surface. A pre-existing test (`test_research_loop.py::test_config_problem_names_must_be_unique`) used `pde="heat"` incidentally while testing an unrelated thing (duplicate-name detection) and was updated to `pde="burgers"`. 6 new tests (2 per config class: reject both unsupported values, accept the `poisson` default). **[2026-09-01 correction, GitHub Copilot review on PR #140]**: the original B24 scope (`heat`/`advection_diffusion`, both lacking any `exact_solution()`) missed `navier_stokes`, which *has* one but returns a vector velocity field of shape `(N, 2)` — incompatible with `BasisSelectionGame`'s scalar `(N,)` fit, so `solution - exact` fails with a numpy broadcasting error rather than the loud `ExactSolutionUnavailableError` this row exists to produce. Verified before fixing: reproduced the exact `ValueError: operands could not be broadcast together with shapes (500,) (500,2)`, then exhaustively checked every other PDE reachable from each Literal (`poisson`, `burgers`, `poisson_lshaped`, `helmholtz`, `biharmonic` — all scalar `(N,)`, confirmed safe). Fixed by renaming each file's rejection frozenset to `_PDES_INCOMPATIBLE_WITH_BASIS_SELECTION` and adding `"navier_stokes"`; `ScalingLawConfig.pde` and `ResearchProblemSpec.pde` already had `navier_stokes` in their Literal so their existing validator covers it directly, but `LLMPriorAblationConfig` only exposes `navier_stokes` via `ood_pde` (not `id_pde`), so a new `_ood_pde_is_compatible` validator was added there. 3 pre-existing tests' `match="ExactSolutionUnavailableError"` no longer matched the rewritten, more general error message and were updated to `match="incompatible with BasisSelectionGame"`; new parametrized `navier_stokes`-rejection and accepts-every-other-option tests added to all three test files. | S-M | Low | `pytest tests/poc/test_llm_prior_ablation_config.py tests/poc/test_scaling_law_config.py tests/agents/test_research_loop.py -v` |
| ~~B25~~ ✅ **DONE (2026-09-01)** | **`GumbelMCTS` backed up child values in the wrong perspective.** GitHub Copilot review on PR #140 (`gumbel.py:537`) found that `_sequential_halving`'s `_simulate(child)` call returns a value from `child.state.current_player`'s perspective — the player to move *at the child*, i.e. root's opponent one ply below root (`apply_action` always switches the mover; `GumbelMCTS` has no `SearchMode`/single-agent concept at all, unlike `src/mcts/search.py`'s F0-fixed `invert_on_backup`, so it always drives standard two-player alternating games) — but the value was backed up into `child.value_sum` **unnegated**. Every downstream consumer (root's own action-selection score, `_gumbel_mixed_value`'s mix with `raw_value`) assumes root's perspective, so an action that was a *guaranteed win for root* scored identically to a *guaranteed loss* — search could actively prefer losing actions. This predates this PR (confirmed via `git diff` against the merge-base: the unnegated `child.value_sum += value` line was already there; this PR's own `_gumbel_mixed_value` addition only made the inconsistency more visible by directly combining `child.value` with `raw_value`, two different perspectives). Fixed by negating `_simulate`'s return before backing it up (`root_perspective_value = -value`, one line in `_sequential_halving`). Three pre-existing tests (`test_discount_scales_the_backed_up_value`, `test_discount_default_is_a_backward_compatible_noop`, `test_use_mixed_value_toggle_changes_best_action`) had their `_terminal_value`/`visited_value` input literals sign-flipped (their asserted output numbers are unchanged — those inputs represent the child's own-perspective value, which the negation now converts before use) with docstrings explaining why. New regression test `TestSequentialHalvingRootPerspective::test_search_prefers_the_action_that_wins_for_root` (`tests/mcts/test_gumbel_integration.py`) constructs exactly the reproduction the review requested — two terminal root actions whose winners differ — and is mutation-tested: reverting the fix flips its `best_action` assertion from 0 to 1. `mypy --strict` error set byte-identical (8 pre-existing errors, unrelated files); full `tests/mcts/` (245 tests) green. | S | Low | `pytest tests/mcts/test_gumbel.py tests/mcts/test_gumbel_integration.py tests/mcts/test_backup_modes.py tests/mcts/test_search.py -v` |
| ~~B26~~ ✅ **DONE (2026-09-01)** | **`add-coverage-gate` and `openspec-change` skills gave conflicting process guidance for the same action.** A GitHub Copilot review on PR #140 flagged the `src/poc/visualization` row added to `openspec/specs/project-charter/spec.md`'s gates register (B20 close-out) as a direct charter edit that should have gone through `openspec-change`'s proposal/design/tasks/delta-spec process (its own table lists "adds or changes a coverage gate" under "use this"), contradicting the PR description's "no Requirement changed" claim. Investigated rather than reverted on the finding alone: `add-coverage-gate` SKILL.md's own Step 5 explicitly instructs editing the charter gates register *directly* as one of five coupled mechanical edits (measure → threshold → `ci.yml` step → docs → verify) — so this PR followed an existing, applicable, more-specific skill exactly as written, and the two skills simply disagreed with each other; neither the *Quality Gate Fidelity* Requirement's text nor its scenario changed (only a guard-verified data row was added to an already-existing, CI-mirroring table, confirmed still green under `tests/docs/test_charter_alignment.py`). Root-caused and fixed the actual defect — the unreconciled skills, not the charter edit itself: `openspec-change` SKILL.md's routing table now carves out "a new package's row at its measured `floor(measured)-2` threshold, CI step already landed" as `add-coverage-gate`'s job, with a paragraph explaining why (mechanical + guard-verified + no Requirement-text change = not the governance-theater case the full package exists for), reserving the full process for a genuine policy call (raising the 85 ceiling, dropping a gate, changing the ⊆-direction rule); `add-coverage-gate` SKILL.md's Step 5 gained a one-line cross-reference so a future reader doesn't have to re-derive this distinction from scratch. `tests/claude/` (87 tests, the harness's own executable-configuration guard) and `tests/docs/` (42 tests, including the charter alignment guard) both green after the edit; `check_doc_links.py` clean. | S | Low | `pytest tests/claude/ tests/docs/ -v` |
| ~~B27~~ ✅ **DONE (2026-09-01)** | **`docs/doe_genesis/mdp_specification.md`'s `MeshRefinementGame` line citations were stale, and had been for a long time.** GitHub Copilot review on PR #140 flagged two of them (`mesh_refinement.py:321`/`:540`, dangling after the B4 mesh_refinement split) and suggested retargeting to `mesh_refinement/game.py:41`/`:420`. Investigated before applying the suggestion rather than trusting it blindly: diffed against the pre-split flat file at the merge-base commit (`d212db7`) and found `class MeshRefinementGame` was already at line 415 there, not the doc's cited 321 — every mesh-related citation in this doc (`get_valid_actions`, `apply_action`, `is_terminal`, `_subdivide_element`, `refine_element`, `_interpolate_solution`, `get_reward`) was off by 30-350+ lines **before this PR touched the file at all**, evidently drifted from a much older version of the module across several unrelated intervening refactors. Copilot's two suggested numbers (`:41`, `:420`) happened to be exactly right for the ones it checked, purely because they matched this PR's own split output — but retargeting only those two would have left 7+ other citations in the same section equally wrong, which is a worse outcome than the finding itself (a doc that looks freshly reviewed while still being mostly incorrect). Fixed all of them against the actual current split files (`mesh_refinement/game.py`, `mesh_refinement/mesh.py`), verified by direct `Read`/`Grep` of each cited symbol's real line, not by re-deriving expected numbers from the doc's prose. `basis_selection.py`/`mcts_adapter.py` citations elsewhere in the same document were left untouched — their staleness (if any) predates and is unrelated to the mesh_refinement split, and fixing them is outside what this PR's own change caused. | S | Low | `python scripts/check_doc_links.py`; manual verification via `grep -n "def <symbol>" src/pde/games/mesh_refinement/{mesh,game}.py` against each citation |
| ~~B28~~ ✅ **DONE (2026-09-02)** | **`src/pde` coverage gate carried 18 points of slack.** A next-steps review (`docs/NEXT_STEPS_REVIEW_2026-08-18.md`; independently re-verified by two specialist deep-dives and two adversarial peer reviews, see the VP-level next-steps case this closes out) found the CI gate (`.github/workflows/ci.yml`'s `Per-module coverage gate (pde)` step) and the charter's gates register (`openspec/specs/project-charter/spec.md`) both still cited `75`, unchanged since 2026-04-10, while `pytest tests/pde/ --cov=src/pde --cov-branch -q` measures **93.5%** (two independent runs: 93.21%, 93.54%). A prior PR (#57, opened 2026-04-27, still open and merge-conflicted 4+ months later) claimed to raise this gate to 85 but never landed — confirmed by reading both files directly rather than trusting the PR description. Raised to **85** (the project ceiling; per `add-coverage-gate` SKILL.md's step 2, never above 85 even when measured higher) in both files. This is the mechanical `add-coverage-gate` case, not a policy call — see B26's carve-out (the project ceiling itself is unchanged). | S | Low | `pytest tests/pde/ --cov=src/pde --cov-branch --cov-fail-under=85 -q --no-header`; `python -m pytest tests/docs/ -v` |
| ~~B29~~ ✅ **DONE (2026-09-02)** | **README.md's Near-Term checklist had two stale items**, surfaced by the same next-steps case. "Migrate Trainer and OperatorTrainer to BaseTrainer inheritance" was half-done and half-not: `src/training/trainer.py::Trainer` already inherits `BaseTrainer` (confirmed via `git blame`, in place by 2026-04-10, 9 days after the checklist line was written and never ticked), and `DistributedTrainer` does too; `OperatorTrainer` was never migrated, but investigation found it has **zero production call sites** (only a re-export from `src/training/__init__.py` and its `hf_space/` mirror) — migrating a class nothing calls is not worth the feature-parity redesign it would require (no AMP today; `BaseTrainer` has no `OneCycleLR` path; incompatible dataclass-vs-Pydantic config; incompatible checkpoint dict shape). "Multi-field PDE support (extending ModelOutput for vector fields)" was stated as if unstarted, but `ModelOutput.vector_fields`/`field_metadata`/`with_vector_fields()`/`has_vector_fields()` already exist (`src/modeling/model.py`, landed 2026-04-26) — unconsumed by any loss/evaluator/forward pass, which is the actually-open part. Both items reworded to state the true remaining scope instead of the stale binary checkbox; `Trainer`'s half of item 2 checked off. | S | Low | `grep -n "^class Trainer\|^class OperatorTrainer" src/training/trainer.py src/training/operator_trainer.py`; `grep -rn vector_fields src/ \| grep -v modeling/model.py` (expect empty) |
| ~~B30~~ ✅ **DONE (2026-09-02)** | **`OperatorTrainer.load_checkpoint` had zero test coverage, and its two existing test files were invisible to the `src/training` CI gate.** Surfaced by the same next-steps case's specialist deep-dive (B29): `tests/test_operator_training.py` and `tests/security/test_checkpoint_roundtrip.py::TestOperatorTrainerRoundTrip` both exercise `OperatorTrainer` but live outside `tests/training/`, so `pytest tests/training/ --cov=src/training --cov-fail-under=85` never collects them — the module measured 25% in that scope, 88% with its own tests. Worse, the existing `TestOperatorTrainerRoundTrip` tests `load_torch_checkpoint` directly against hand-built payload dicts and never actually constructs an `OperatorTrainer` or calls `.load_checkpoint()` — the state-restoration lines (`current_epoch`, `best_val_loss`, `history`, model/optimizer state dicts) and the `allow_unsafe_pickle` keyword-only opt-in (added 2026-08-21 for exactly this loader) were untested by anything. Fixed with two new tests in `tests/test_operator_training.py::TestOperatorTrainer`: `test_load_checkpoint_restores_state` (a real save → fresh-trainer → load round trip through the public API, asserting epoch/best_val_loss/history/model-weights all transfer) and `test_load_checkpoint_allow_unsafe_pickle_flag_is_plumbed_through` (default `False` rejects a legacy raw-dataclass payload; `allow_unsafe_pickle=True` on the trainer's own method — not `load_torch_checkpoint` directly — lets the identical file load). Rather than moving the test files into `tests/training/` (which would add ~6s to that job, not the ~318s a coarser measurement first suggested — re-measured after adding the new tests) or marking them `slow`, wired a dedicated native-runner CI step scoped to just this file (matching the existing `poc/cli.py` per-file gate's pattern), gated at 85 against the measured 88%. | S | Low | `python -m coverage run --branch --include="*/src/training/operator_trainer.py" -m pytest tests/test_operator_training.py tests/security/test_checkpoint_roundtrip.py::TestOperatorTrainerRoundTrip -q -p no:cov && python -m coverage report --include="*/src/training/operator_trainer.py" --fail-under=85` |
...
|---|---|---|---|
| src/research/fem_baseline.py | 85.0 | 51 | **Gated at 83** in `test-extras` (2026-09-02). Was in the global `omit` (optional `[fem]` extra) and therefore invisible to the `--cov=src/research` gate at 85 that it lives under — 249 statements contributing nothing to a package percentage that looked healthy. Surfaced automatically by the new `tests/docs/test_coverage_gate_integrity.py`, which is the point of that guard: this class of false pass had been found three times by hand and never by a check. |
| src/research/substrates | 99.0 | 100 | **Gated at 95** in the `test-extras` job (2026-09-02), not the `coverage` job: `skfem_tri.py` needs the optional `[fem]` extra and is in pyproject.toml's global `omit`, so a bare `--cov` of it reports 0.00% with `CoverageWarning: No data was collected` — the same false pass this file already records for `video_compression` and `demos`, and the reason the previously-documented "89%" for that module was measured and enforced nowhere. Same inline-`.coveragerc` fix. Gate deliberately **above** the usual `floor(measured)-2 capped at 85` convention: the package is 272 statements, so an 85 gate would carry 14 points of slack and reproduce the finding. |
...
| src/templates | 72.5 | 107 | **cli.py 0% (121 stmts)** — triage before gating. **Parked 2026-09-10** (hygiene-hardening-cycle Wave D): do not land a 70-ish gate. |
| src/math_kernel | 61.5 | 155 | uniform 55–64% — triage before gating. **Parked 2026-09-10**: JAX-class arcs need `[jax]` tests, not a 59 gate. |
...
| src/integrations/eval_harness | **98.16%** (CI run 34658949836, 2026-09-11) | 39 collected, all `eval_harness_required` | **B37 closed 2026-09-11 (R-13)**: gated in `test-extras` at **96** = floor(98)-2 (inline coveragerc overrides the omit; `-m "eval_harness_required"` selects the whole suite). Landed as a `1` tripwire; the first measured run also exposed the `function:`/`path` contract defect in the shipped YAML. |
...
**[2026-09-02 correction: now 2 of 8.]** The Burgers link below was fixed 2026-08-19 (see the
`Status (2026-08-21)` block further down) — its OOD reward is no longer flat, verified at runtime
through the real `_centaur_common` path (six distinct rewards, monotone error decrease from a
non-degenerate 0.7071 initial error). `HeatOperator` and `AdvectionDiffusionOperator` remain open;
see that status block for the current per-operator state. The original 3-of-8 investigation below
is left as written for provenance; `src/pde/operators.py` (link 1's citation) no longer exists —
it was split into the `src/pde/operators/` package (`burgers.py`, `heat.py`,
`advection_diffusion.py`, ...) on 2026-09-01 (B4), with the described bug and fix now at
`src/pde/operators/burgers.py`.

**Independently verified by execution, not inspection.** Three links, each confirmed:

1. `src/pde/operators.py` (now `src/pde/operators/burgers.py`) — `BurgersOperator.__init__` assigns
...
- ~~**`COVERAGE_CORE=pytrace` was missing from the documented gate commands.** The failure mode is
  ...
  **RETRACTED 2026-09-02 (B36).** Re-measured on the identical `src/training` gate: **88.25%
  under both tracers**, byte-identical per file, `base_trainer.py` **88% either way**. The
  89.53/82.45 pair does not reproduce, and neither does `cb645ad`'s separate "crashes at
  collection" claim. The pin was retired repo-wide; the warning that headed the table is now a
  retraction. Kept here rather than deleted because the measurement was recorded in good faith and
  the record of *how* a false invariant entered the repo is the useful part.
