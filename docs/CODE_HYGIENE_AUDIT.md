...
| B7 | CI composite action for the ×10 checkout/setup/install preamble + a shared `--ignore`/`--deselect` args file (currently duplicated verbatim between `test-fast` and `coverage`). **The third sub-item, `COVERAGE_CORE` centralization, is closed 2026-09-02 by deletion rather than centralization** — there is nothing left to centralize, see B36 | M | Low-Med | Preserve step names — the charter's Quality-Gate guard parses `ci.yml` by `- name:` |
