...
| B12 | AGENT.md authoring for the uncovered packages, starting with `src/research` (§3.7) | M | None | `check_doc_links.py`. **[2026-09-02: re-counted — **15 of 28**, not 14 of 26; the old figure predated `src/core` and `src/refinement`. `src/refinement/AGENT.md` written (the package the element-local-substrate work created — documenting the domain-free contract, the numpy-only constraint on `substrate.py`, the SINGLE_AGENT requirement, and the self-expiring staged-exemption rule). 14 remain; `src/research` (~9.9k LOC) is still the largest.]** |
