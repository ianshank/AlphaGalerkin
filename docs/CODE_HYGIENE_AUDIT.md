...
| B23 | **PR and CI schedule hygiene.** Two small, low-risk items observed while driving PR #140 to green: (1) the nightly `schedule`-triggered CI run on the default branch has self-cancelled on an unchanged commit for at least 9 consecutive days (2026-08-24 through 2026-09-01, verified via the GitHub Actions API's workflow-run listing for the `CI` workflow — not the `gh` CLI, which is not available in every environment this repo is worked from) — real runner minutes spent for zero new signal, likely fixable with a `paths` guard or a check for "HEAD unchanged since last scheduled run" before the matrix fans out; (2) 4 open PRs (#48, #57 from April; #118, #128 from August) target base commits that are no longer the branch tip and have had no activity in 1-4+ weeks — a lightweight monthly Routine that lists open PRs, flags ones whose base SHA predates the current tip by more than N commits or with no activity in 30+ days, and comments/labels them would surface this automatically instead of relying on someone noticing during an unrelated task. | S | Low | `.github/workflows/ci.yml`'s `schedule` trigger for (1); a new Routine + the GitHub PR-listing/commit-comparison API for (2) |
| B24 | **Graceful arm-skip for `BasisSelectionGame` operators with no exact solution.** PR #140's `ExactSolutionUnavailableError` fix (P0-1 step 2) correctly turns a silent, always-converged lie into a loud crash for `heat`/`advection_diffusion` — but three config surfaces (`src/poc/scenarios/llm_prior_config.py::id_pde`, `src/poc/scenarios/scaling_law_config.py::PDEName`, `src/agents/config.py::ResearchPDEName`) still schema-advertise both as legal selections with no validator or graceful-skip, so a user who picks one gets an opaque scenario `ERROR` instead of either a validation error or a clean arm-skip. The codebase already has the pattern to copy — the same three scenario families disable the LLM arm gracefully on preflight failure (CLAUDE.md's LLM-Prior milestone: "the affected arm is disabled and its acceptance thresholds are removed from `self.config.thresholds`"). Not fixed in #140 itself: this is a 3-module change (each scenario's `setup()` plus its threshold-handling), not a small local fix, and it's confirmed non-regressing today (no shipped YAML sets either value). Each Literal now carries a `NOTE:` comment pointing here in the meantime. Peer-review finding, PR #140. | S-M | Low | Would need a test per scenario family proving the graceful skip, mirroring the existing LLM-preflight-failure tests |
