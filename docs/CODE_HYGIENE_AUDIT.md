...
| B23 | **PR and CI schedule hygiene.** Two small, low-risk items observed while driving PR #140 to green: (1) the nightly `schedule`-triggered CI run on the default branch has self-cancelled on an unchanged commit for at least 9 consecutive days (2026-08-24 through 2026-09-01, `gh run list` on the `CI` workflow) — real runner minutes spent for zero new signal, likely fixable with a `paths` guard or a check for "HEAD unchanged since last scheduled run" before the matrix fans out (verified via the GitHub Actions API's workflow-run listing, not the `gh` CLI, which is not available in every environment this repo is worked from); (2) 4 open PRs (#48, #57 from April; #118, #128 from August) target base commits that are no longer the branch tip and have had no activity in 1-4+ weeks — a lightweight monthly Routine that lists open PRs, flags ones whose base SHA predates the current tip by more than N commits or with no activity in 30+ days, and comments/labels them would surface this automatically instead of relying on someone noticing during an unrelated task. | S | Low | `.github/workflows/ci.yml`'s `schedule` trigger for (1); a new Routine + the GitHub PR-listing/commit-comparison API for (2) |
