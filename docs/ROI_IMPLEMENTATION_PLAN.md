...
**Status Update (2026-04-25):** **Learned PDE Evaluator wired** — `AlphaGalerkinConfig.evaluator="trained"` is back in the typed Literal and dispatches through `FNetEvaluator` with checkpoint loading via `create_model_from_checkpoint`. Closes the only non-trivial *Known Issue* and makes `train_pde.yaml` a closed loop. The trained-evaluator path is cached on the solver instance (one disk load per solver, invalidated via `reset_cache()`) and the device-resolution helper is `lru_cache`-backed so benchmark suites don't spam downgrade warnings. Default device flipped from `cpu` to `cuda` with automatic CPU fallback. Per-module coverage on `src/alphagalerkin/` raised to 94%. See PR #54 for the diff.
