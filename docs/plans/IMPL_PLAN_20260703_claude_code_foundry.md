...
1. Every copied file gets a **format-aware** generated-file header warning against
   manual edits: an HTML comment (`<!-- GENERATED by foundry vX.Y.Z — do not edit -->`)
   for Markdown, a metadata field for JSON (where comments are invalid syntax), and a
   `# GENERATED …` comment for Python/shell scripts.
...
5. **CLAUDE.md block (idempotency):** delimited by **static, version-agnostic**
   sentinels (`<!-- foundry:begin --> … <!-- foundry:end -->`) with the version
   recorded *inside* the block, so upgrades locate the boundaries with a fixed string
   match instead of parsing variable version tags; scaffold replaces only between
   sentinels; golden-file tests for the rendered block.
...
third-person voice via lightweight regex heuristics (e.g. description starts with a
configurable list of third-person verbs — no NLP dependencies), configurable jargon
denylist, sidecar fixture existence + minimum corpus size. `foundry docs` (F10): README component tables rendered from the parsed
manifest, golden-file tested. *Exit:* both green over the Phase 2 set; CI wired.

**Phase 5 — Release v0.1.0 (F6).**
CHANGELOG; CI job (running on tag events only, so ordinary commits are never blocked
by the absence of a tag) asserting `plugin.json version == git tag == CHANGELOG top
entry`; release workflow. **Live smoke runbook** (release-blocking, manual): fresh clone →
...
foundry lint-descriptions → [on tag only] version-consistency
(tag == plugin.json == CHANGELOG) → [on tag] release
```
...
- [ ] plugin.json version == git tag == CHANGELOG top entry (CI-enforced on tag events)
