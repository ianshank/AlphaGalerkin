...
- [x] 1.1 Add concrete game module **outside** `src/refinement/` (e.g.
      `src/pde/games/substrate_refinement.py`) implementing `RefinementGame` over a
      `RefinementSubstrate`
- [x] 1.2 Pure `apply_action(state, action)` — no instance mutation; mesh via fingerprint /
      immutable handle
- [x] 1.3 Map `SubstrateSolveResult` → `RefinementState` (error, dofs, indicators, budget/step)
- [x] 1.4 Action space: per-element (or documented top-1) mark → `refine`; reject invalid ids
- [x] 1.5 Unit tests: purity, terminal/reward smoke, tensor export shape

## 2. Registration (never `__init__`)

- [x] 2.1 `@register_refinement_game("…")` on the concrete class
- [x] 2.2 Explicit `register_*` module (e.g. `src/pde/register_refinement_games.py`); document
      import side-effect pattern
- [x] 2.3 Import-graph test: package `__init__` files do not register refinement games
- [x] 2.4 Registry clear setup/teardown (or subprocess) per `src/refinement/AGENT.md`

## 3. Substrate lookup + fingerprint cache

- [x] 3.1 First non-test `RefinementSubstrateRegistry` lookup by config `kind`
- [x] 3.2 Fingerprint-keyed solve cache bounded by `solve_cache_max_entries`
- [x] 3.3 Drop `("RefinementSubstrate", "fingerprint")` from
      `scripts/audit_abstractions.py::_STAGED_FOR_UPCOMING_TASK`
- [x] 3.4 Ensure `src/research/substrates/` concretes are imported from the register module (not
      only via direct test construction)

## 4. Adapter smoke (not the arena)

- [x] 4.1 Wire `RefinementGameAdapter` for a single short search smoke (CPU tensor_grid and/or
      `fem_required` skfem)
- [x] 4.2 Assert smoke does **not** publish README/charter headline numbers

## 5. Governance / charter / docs

- [x] 5.1 Retire `RefinementGameRegistry`-has-no-registrants deviation (was parent task 8.1b)
- [x] 5.2 Add time-boxed two-path deviation (legacy L-shape harness vs substrate game) with
