...
| `RefinementSubstrateRegistry` has zero runtime registrants | Same shape as the row above, and disclosed rather than left for a reviewer to find: neither `TensorGridSubstrate` nor `SkfemTriSubstrate` carries `@register_refinement_substrate`, so `RefinementSubstrateRegistry().get_or_raise("skfem_tri")` fails for any real caller. Both substrates are constructed directly today (by the adequacy gate and by tests); the registry exists because `element-local-substrate` Slice E introduces config-driven substrate selection, which is the first thing that needs a lookup. Retires when that lands — or the registry goes. |
