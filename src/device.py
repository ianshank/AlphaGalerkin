"""Shared device-resolution helper (GPU-preferred, fail-loud).

The project default is GPU-preferred: callers pass ``device='cuda'`` and
...
Kept as a plain module (a sibling to ``src/constants.py`` and
``src/seeding.py``) rather than a package so it does not enlarge the
drift-guarded package map enforced by
``tests/docs/test_architecture_map.py`` (which enumerates ``src/*/__init__.py``).

``src.poc.device`` re-exports this module identically so existing
``from src.poc.device import resolve_device`` call sites — including the
frozen codec track — keep working without an import migration.
