...
class QuickEvaluator:
    ...
    def _compute_mse(predictions: list[float], targets: list[float]) -> float:
        ...
        if not predictions:
            return 0.0
        return sum((p - t) ** 2 for p, t in zip(predictions, targets, strict=False)) / len(
            predictions
        )
    ...
    def _compute_mae(predictions: list[float], targets: list[float]) -> float:
        ...
        if not predictions:
            return 0.0
        return sum(abs(p - t) for p, t in zip(predictions, targets, strict=False)) / len(
            predictions
        )
    ...
    def _compute_accuracy(predictions: list[float], targets: list[float]) -> float:
        ...
        correct = sum(1 for p, t in zip(predictions, targets, strict=False) if round(p) == round(t))
    ...
    def _compute_r2(predictions: list[float], targets: list[float]) -> float:
        ...
        ss_res = sum((t - p) ** 2 for p, t in zip(predictions, targets, strict=False))
        if ss_tot == 0:
