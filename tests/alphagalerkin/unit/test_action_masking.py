...
class TestActionMasking:
    ...
    def test_budget_exceeded_blocks_refinement(
        ...
        refine_actions = [a for a in actions if a.action_type == ActionType.H_REFINE]
    ...
    def test_min_element_size_blocks_h_refine(
        ...
        masker = ActionMasker(config)
        actions = masker.valid_actions(initial_state)
        h_refine = [a for a in actions if a.action_type == ActionType.H_REFINE]
    ...
    def test_max_poly_order_blocks_p_refine(
        ...
        masker = ActionMasker(config)
        actions = masker.valid_actions(initial_state)
        p_refine = [a for a in actions if a.action_type == ActionType.P_REFINE]
    ...
    def test_coarsen_blocked_on_initial_mesh(
        ...
        masker = ActionMasker(config)
        actions = masker.valid_actions(initial_state)
        coarsen = [a for a in actions if a.action_type == ActionType.H_COARSEN]
    ...
    def test_noop_always_available(
        ...
        masker = ActionMasker(config)
        actions = masker.valid_actions(initial_state)
        noop = [a for a in actions if a.action_type == ActionType.NO_OP]
        assert len(noop) >= 1
    ...
    def test_p_coarsen_blocked_at_order_one(
        ...
        masker = ActionMasker(config)
        actions = masker.valid_actions(initial_state)
        p_coarsen = [a for a in actions if a.action_type == ActionType.P_COARSEN]
