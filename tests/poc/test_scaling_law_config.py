...
def test_pde_rejects_operators_incompatible_with_basis_selection(unsupported_pde: str) -> None:
    """B24 (+Copilot-review navier_stokes correction, PR #140).

    Fail fast at construction, not mid-sweep via ExactSolutionUnavailableError
...
def test_pde_accepts_every_other_option(pde: str) -> None:
    cfg = ScalingLawConfig(pde=pde)  # type: ignore[arg-type]
    assert cfg.pde == pde
