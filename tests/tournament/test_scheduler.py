...
class TestTournamentScheduler:
    ...
    def test_generate_pairings_empty(self, scheduler: TournamentScheduler) -> None:
...
class TestRoundRobinScheduler:
    ...
    def test_round_robin_odd_players(self, scheduler: TournamentScheduler) -> None:
        """Test round-robin with odd number of players."""
        players = [Player(name=f"P{i}", player_id=f"p{i}") for i in range(5)]
...
class TestSwissScheduler:
    ...
    def test_swiss_avoids_repeat_pairings(self, swiss_scheduler: TournamentScheduler) -> None:
...
class TestEliminationScheduler:
    ...
    def test_single_elimination_pairings(self, elimination_config: TournamentConfig) -> None:
        """Test single elimination pairings."""
        scheduler = TournamentScheduler(config=elimination_config)
        players = [
            Player(name=f"P{i}", player_id=f"p{i}", rating=1500.0 + i * 100) for i in range(4)
        ]
    ...
    def test_elimination_seeds_by_rating(self, elimination_config: TournamentConfig) -> None:
    ...
    def test_elimination_byes(self, elimination_config: TournamentConfig) -> None:
        """Test elimination with byes for non-power-of-2."""
        scheduler = TournamentScheduler(config=elimination_config)
        players = [Player(name=f"P{i}", player_id=f"p{i}", rating=1500.0) for i in range(3)]
...
class TestGetNextRound:
    ...
    def test_swiss_next_round(
        ...
        # Get next round
        round2 = swiss_scheduler.get_next_round(sample_players, matches)

        assert len(round2) == 2

    def test_elimination_next_round(self, elimination_config: TournamentConfig) -> None:
